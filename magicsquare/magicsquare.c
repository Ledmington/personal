/*
		...
		d e f
		g h i
		such that the sum of all elements squared in each row, column and
   diagonal is always the same. More formally: a,b,c,d,e,f,g,h,i are all
   positive integers a*a + b*b + c*c = k d*d + e*e + f*f = k g*g + h*h + i*i = k
		a*a + d*d + g*g = k
		b*b + e*e + h*h = k
		c*c + f*f + i*i = k
		a*a + e*e + i*i = k
...
uint usqrt(uint x) { return (uint)floor(sqrt((float)x)); }
...
bool unpack(PackedMagicSquare pms, MagicSquare *sq) {
	...
	*/
	if (!(pms.v[0] < pms.v[1] && pms.v[1] < pms.v[2] && pms.v[2] < pms.v[3])) {
		return 0;
	}
	...
	if (!issquare(g) || !issquare(e) || !issquare(h) || !issquare(f) ||
		!issquare(i)) {
		return 0;
	}
...
#pragma omp parallel num_threads(nthreads + 1)
		...
		if (omp_get_thread_num() == 0) {
			while (1) {
				...
				for (int t = 0; t < nthreads; t++) {
					if (v[t].finished) {
						printf(
							" [%2d] %10llu checked, %10llu found (finished)\n",
							t, v[t].i, v[t].found);
						count++;
						continue;
					}
					printf(
						" [%2d] %10llu checked, %10llu found, %10llu remaining "
						"(%.3f%%)\n",
						t, v[t].i, v[t].found,
						(v[t].end - v[t].start + 1) - v[t].i,
						(double)v[t].i / (double)(v[t].end - v[t].start + 1) *
							100);
				}
	...
	printf(
		...
		nthreads, (double)t, limit, found, (double)found / (double)limit * 100);
