...
#pragma omp parallel default(none) shared(l_coll, circles)
	{
...
int main(const int argc, const char* argv[]) {
	...
	if (argc > 2) {
		...
		if (ncircles < 0 || ncircles > max_ncircles) {
			fprintf(
				stderr,
				"The number of points must be positive and smaller than %d\n\n",
				max_ncircles);
			return EXIT_FAILURE;
		}
	}
	...
	print_ram_usage(N_WIDTH * sizeof(int**) +
					N_WIDTH * N_HEIGHT * sizeof(int*) +
					N_WIDTH * N_HEIGHT * MAX_CIRCLES * sizeof(int));
	fprintf(stderr, "\n\n");
