...
void naive_matmul_2d(data_type **a, data_type **b, data_type **c,
...
void naive_matmul_1d(data_type *a, data_type *b, data_type *c,
					 const index_type size) {
	for (index_type i{0}; i < size; i++) {
		for (index_type k{0}; k < size; k++) {
...
HWY_ATTR void vector_matmul_2d(data_type **HWY_RESTRICT a,
							   data_type **HWY_RESTRICT b,
							   data_type **HWY_RESTRICT c,
							   const index_type size) {
...
HWY_ATTR void vector_matmul_1d(data_type *HWY_RESTRICT a,
							   data_type *HWY_RESTRICT b,
							   data_type *HWY_RESTRICT c,
							   const index_type size) {
	namespace hn = hwy::HWY_NAMESPACE;
...
uint64_t get_timer_resolution() {
	...
	struct std::chrono::time_point<std::chrono::high_resolution_clock>
		v[length];
	...
	uint64_t res = static_cast<uint64_t>(
		std::chrono::duration_cast<std::chrono::nanoseconds>(v[1] - v[0])
			.count());
...
void init_2d(data_type **a, data_type **b, data_type **c,
			 const index_type size) {
	for (index_type i{0}; i < size; i++) {
		for (index_type j{0}; j < size; j++) {
			a[i][j] = dist(rng);
	...
	for (index_type i{0}; i < size; i++) {
		for (index_type j{0}; j < size; j++) {
			if (c[i][j] < 0 || c[i][j] > static_cast<data_type>(size)) {
				std::cout << "ERROR: invalid result at index " << i << "; " << j
						  << std::endl;
			}
		}
	}
...
double nano_to_seconds(const double ns) { return ns / 1000000000; }
...
void run_benchmarks(const uint32_t size, const uint32_t max_iterations) {
	...
	std::cout << "     | Naive 2D | Vector 2D | Naive 1D | Vector 1D |"
			  << std::endl;
...
int main(const int argc, const char **argv) {
	...
	if (argc > 3) {
		std::cout
			<< "WARNING: more than 2 arguments provided, ignoring the others."
			<< std::endl;
	}
	...
	printf("Approx. total memory used : %.3f MB\n",
		   (double)(6 * (uint64_t)size * (uint64_t)size * sizeof(data_type)) /
			   1048576.0);
	printf(
		"Vector type               : %ld x %ld bits\n",
		hwy::HWY_NAMESPACE::Lanes(hwy::HWY_NAMESPACE::ScalableTag<data_type>{}),
		8 * sizeof(data_type));
	printf("\n");
