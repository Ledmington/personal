...
int main ( int argc, char *argv[] ) {
    ...
    for(int i=0; i<16; i++){
        ...
        griglia[i].adj = (struct s2**) malloc(griglia[i].num_adj * sizeof(struct s2*));
    ...
    while( (ch = fgetc(fin)) != EOF ){
        if(ch == '\n'){
            lengths = (unsigned int*) realloc(lengths, (++num_words)*sizeof(unsigned int));
            lengths[num_words-1] = 0;
        }
        else lengths[num_words-1]++;
    }
