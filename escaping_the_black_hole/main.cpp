...
match_result simulate_match(const std::vector<card>& original_deck, const size_t n_players,
	...
	for (; turn <= max_turns; turn++) {
		...
		std::cout << game.deck.size() << " cards in the deck." << std::endl;
		std::cout << game.discard_pile.size() << " cards in the discard pile." << std::endl;
		std::cout << "Total cards: "
				  << (game.deck.size() + game.discard_pile.size() +
					  std::accumulate(game.players.begin(), game.players.end(), 0,
									  [](size_t sum, const player& p) { return sum + p.hand.size(); }))
				  << std::endl;
		std::cout << count_alive_players(game) << " alive players." << std::endl;
#endif	// LOGGING
		...
		for (size_t p{0}; p < n_players; p++) {
...
#if defined(LOGGING) && LOGGING == 1
			std::cout << " " << players_names.at(p) << "'s turn" << std::endl;
#endif	// LOGGING
			...
			do {
#if defined(LOGGING) && LOGGING == 1
				std::cout << "  Hand is: " << get_string(game.players.at(p).hand) << std::endl;
#endif	// LOGGING

				if (contains(game.players.at(p).hand, card::METEOR_SHOWER)) {
#if defined(LOGGING) && LOGGING == 1
					std::cout << "  Plays " << ANSI_BOLD << card::METEOR_SHOWER << ANSI_RESET << "." << std::endl;
#endif	// LOGGING
...
#if defined(LOGGING) && LOGGING == 1
				std::cout << "  Available moves: " << get_string(moves) << std::endl;
#endif	// LOGGING
...
#if defined(LOGGING) && LOGGING == 1
					std::cout << "  Plays " << ANSI_BOLD << card_to_play << ANSI_RESET << "." << std::endl;
#endif	// LOGGING
...
#pragma omp parallel for schedule(auto) default(none)                       \
	...
	std::cout << " First Turn Victories:    " << std::setw(7) << first_turn_victories << " / " << std::setw(7)
			  ...
			  << "%)." << std::endl;
	std::cout << " Games ended by Asteroid: " << std::setw(7) << games_ended_by_meteor_shower << " / " << std::setw(7)
			  << matches_to_simulate << " (" << std::fixed << std::setprecision(2)
			  << ((static_cast<double>(games_ended_by_meteor_shower) / static_cast<double>(matches_to_simulate)) *
				  100.0)
			  << "%)." << std::endl;
	std::cout << " Games won by Asteroid:   " << std::setw(7) << games_won_by_meteor_shower << " / " << std::setw(7)
			  << matches_to_simulate << " (" << std::fixed << std::setprecision(2)
...
double evaluate(const std::unordered_map<card, size_t>& num_cards, const size_t num_players) {
	...
	const size_t matches_to_simulate = 1'000;
	...
	while (true) {
		...
		if (best_fy < fx) {
			...
			std::cout << " Total cards: "
					  << (std::accumulate(
							 x.begin(), x.end(), 0,
							 [](const size_t sum, const std::pair<card, size_t>& entry) { return sum + entry.second; }))
					  << "." << std::endl;
			for (auto const& [c, num] : x) {
				std::cout << " # " << c << ": " << num << std::endl;
			}
			std::cout << std::endl;
		} else {
