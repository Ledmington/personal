...
color colori[8] = {{"red", false},	  {"yellow", false}, {"green", false},
				   {"orange", false}, {"pink", false},	 {"purple", false},
...
int main(void) {
	...
	while (tentativi && num_rossi != 4) {
		...
		do {
			if (num_gialli + num_rossi > 4 || num_gialli + num_rossi < 0)
...
void print_solutions() {
	...
	tmp = soluzioni;
	while (tmp != NULL) {
		for (int i = 0; i < 4; i++) printf("%s - ", tmp->ordine[i]->name);
		printf("\n");
		tmp = tmp->next;
	}
	printf("---\n");
...
int evaluate_solution(solution* sol) {
	...
	if (num_gialli + num_rossi == 4) {
		if (g + r == 4 && r != 4) {
			sol->score +=
				(float)(abs(num_rossi - r) + abs(num_gialli - g) * 1.1);
			return 1;
		} else
			return 0;
