...
int main(const int argc, const char **argv)
{
    ...
    if (argc > 2)
    {
        fprintf(stdout, "I do not need more than one argument. I'll ignore the others.\n");
    }

    const uint32_t max_length = max_bytes / sizeof(DATA_TYPE);

    fprintf(stdout, " --- Bandiwidth benchmark --- \n");
    fprintf(stdout, "Compiled with %s %d.%d.%d on %s %s\n", COMPILER, COMPILER_MAJOR, COMPILER_MINOR, COMPILER_PATCH, __DATE__, __TIME__);
    fprintf(stdout, "------------------------------\n");
...
#pragma omp atomic
    ...
    for (uint32_t bytes = sizeof(DATA_TYPE); bytes <= max_bytes; bytes *= 2)
    {
        ...
        int values_size = 100;
        int values_length = 0;
        RESULT_TYPE *values = (RESULT_TYPE *)malloc(values_size * sizeof(RESULT_TYPE));
        if (values == NULL)
        {
    ...
    schedule(static)
            ...
            */
            ...
            for (INDEX_TYPE j = 0; j < length; j++)
            {
                if (a[j] != b[j])
                {
                    fprintf(stderr, "ERROR: arrays differ at index %d: %f; %f\n", j, a[j], b[j]);
                    break;
                }
            }
        ...
        printf(CYAN "%7.3f %2s" WHITE " (%12u elements): " GREEN "%7.3f %2s/s" WHITE " +- " YELLOW "%7.3f %2s/s" RESET "\n", format_bytes(bytes), biggest_byte_unit_for(bytes), length, format_bytes(mean_bw), biggest_byte_unit_for(mean_bw), format_bytes(hwci_bw), biggest_byte_unit_for(hwci_bw));
    }
